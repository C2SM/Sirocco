...
class AiidaWorkGraph:
    ...
    def _create_shell_task_node(self, task: core.ShellTask):
        ...
        if task.src is not None:
            if core.enums.Computer(task.computer) == core.enums.Computer.LOCALHOST:
                nodes[f"EXECUTABLE_{label}"] = aiida.orm.SinglefileData(os.path.expandvars(str(task.src)))
            else:
                nodes[f"EXECUTABLE_{label}"] = aiida.orm.RemoteData(str(task.src), computer=task.computer)
