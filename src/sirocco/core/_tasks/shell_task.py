...
class ShellTask(models.ConfigShellTaskSpecs, Task):
    ...
    def build_from_config(cls: type[Self], config: models.ConfigTask, config_rootdir: Path, **kwargs: Any) -> Self:
        ...
        if config.relpath is not None:
            self.path = self._validate_relpath(config.relpath, config_rootdir)
        return self

    @staticmethod
    def _validate_relpath(relpath: Path, config_rootdir: Path) -> Path:
        if relpath.is_absolute():
            msg = f"Script relpath {relpath} must be relative with respect to config file."
        path = config_rootdir / relpath
        if not path.exists():
