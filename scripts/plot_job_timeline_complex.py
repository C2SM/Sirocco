#!/usr/bin/env python3
"""Plot job timeline from AiiDA WorkGraph with custom ordering for complex workflow.

Usage:
    python plot_job_timeline_complex.py <node_pk_or_uuid> [--output timeline.png]
"""
...
def collect_job_data(node):
    ...
    # Custom sorting for complex workflow
    def sort_key(job):
        """Sort jobs by workflow structure: setup, fast branch, medium branch, slow branch, finalize, prepare_next.
        ...
        # Extract task type and cycle date
        # Format: "dynamic_deps_complex_YYYY_MM_DD_HH_MM_SS_taskname_..."
        # Or just: "taskname_date_YYYY_MM_DD_..."
...
def plot_timeline(jobs, output_file=None):
    ...
    # Set up the plot
    fig, ax = plt.subplots(figsize=(16, max(8, len(jobs) * 0.5)))
    ...
    def extract_task_name(full_name):
        ...
        Examples:
        - "dynamic_deps_complex_2026_01_07_12_05_setup" -> "setup"
        - "dynamic_deps_complex_2026_01_07_12_05_fast_1_date_2026_01_01_00_00_00" -> "fast_1"
        """
    ...
    ax.set_title('Complex Workflow Timeline: Branch Dependencies', fontsize=13, fontweight='bold')

    # Grid
    ax.grid(True, axis='x', alpha=0.3)

    # Add subtle horizontal lines to separate task groups
    group_boundaries = []
    prev_task_type = None
    for i, job in enumerate(jobs):
        name = job['name']
        # Determine task type
        for task in ['setup', 'fast_', 'medium_', 'slow_', 'finalize', 'prepare_next']:
            if task in name:
                if prev_task_type and prev_task_type != task:
                    group_boundaries.append(len(jobs) - i - 0.5)
                prev_task_type = task
                break
...
def main():
    parser = argparse.ArgumentParser(
        description='Plot job timeline from AiiDA WorkGraph with custom ordering for complex workflow'
    )
    parser.add_argument(
        'node',
        type=str,
